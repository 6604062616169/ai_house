...
if st.session_state.page == "Machine Learning":
    ...
    st.write(
        ...
        **ตรวจสอบคอลัมน์ของ Train และ Test** \n
          รูป 8 \n  
        เราตรวจสอบว่าคอลัมน์ใน Train และ Test ตรงกันหรือไม่ 
        `X_train`และ`X_test` มี 287 คอลัมน์ ซึ่งหมายความว่าข้อมูลทั้งสองชุด ตรงกันแล้ว
        """
    ) 
    st.write(
        """
    ...
    st.write(
        ...
        **การสร้างไฟล์ Submission และบันทึกโมเดล** \n
          รูป 10 \n
        ใช้โมเดลที่เทรนเสร็จแล้วพยากรณ์ราคาบ้าน  
        และบันทึกผลลัพธ์เป็นไฟล์ CSV เพื่อใช้ส่งผลลัพธ์  
        """
    )
    st.write(
        """
        **โหลดโมเดลที่บันทึกไว้ และตรวจสอบไฟล์ Submission** \n
          รูป 11 \n
        หลังจากที่เราเทรนโมเดลและบันทึกไว้เป็นไฟล์ `house_price_model.pkl`  
        เราสามารถโหลดโมเดลขึ้นมาใช้งานใหม่ได้ โดยไม่ต้องเทรนซ้ำ  

        นอกจากนี้ เรายังตรวจสอบไฟล์ `submission.csv`  
        ซึ่งเป็นไฟล์ที่บันทึกผลลัพธ์การพยากรณ์ราคาบ้าน  
        เพื่อนำไปใช้ส่งผลลัพธ์ หรือใช้งานในขั้นตอนต่อไป  
        """
    )
    st.write(
        """
        **ตรวจสอบจำนวนและชื่อของฟีเจอร์ที่ใช้ในโมเดล** \n
          รูป 12 \n
        ก่อนใช้งานโมเดล เราต้องตรวจสอบว่าจำนวนฟีเจอร์ที่ใช้ตรงกับโมเดลที่บันทึกไว้  
        และดูว่าฟีเจอร์ที่โมเดลใช้มีอะไรบ้าง  
        """
    )
    st.write(
        """
        **ทำนายราคาบ้านด้วยโมเดลที่ฝึกไว้** \n
          รูป 13 \n
        ใส่ค่าพื้นที่ใช้สอย จำนวนห้องนอน และห้องน้ำ  
        จากนั้นโมเดลจะพยากรณ์ราคาบ้านให้คุณ  
        """
    )
